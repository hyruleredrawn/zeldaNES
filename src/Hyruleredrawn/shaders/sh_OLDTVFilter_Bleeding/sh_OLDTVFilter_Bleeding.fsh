...
void main() {
    ...
    for(float x = -2.0; x <= 2.0; x+=1.0 ) {
        px = x * f_screenWidth - f_screenWidth * 0.5;
        yuv.g += RGB2U( texture2D( gm_BaseTexture, v_vTexcoord + vec2(px, 0.0) ).rgb ) * a_kernel[int(x) + 2];
        yuv.b += RGB2V( texture2D( gm_BaseTexture, v_vTexcoord + vec2(px, 0.0) ).rgb ) * a_kernel[int(x) + 2];
    }
